...
void main() {   
	...
	if(mode == 0) {
		...
		bvec3 down = lessThan(_vclr, vec3(adjustedFact.x));
		bvec3 up = lessThan(_vclr2, vec3(adjustedFact.x));
		...
		float sdown = falloff(exp, adjustedFact.y * vdown);
		float sup = falloff(exp, adjustedFact.y * vup);
		...
		float sdown = falloff(exp, adjustedFact.y * _vclr[ind]);
		float sup = falloff(exp, adjustedFact.y * _vclr2[ind]);
